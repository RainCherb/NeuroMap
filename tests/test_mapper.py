...
def test_claude_format():
    """Test CLAUDE.md format for Claude Code."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        ...
        neuromap = scan_and_map(temp_path)
        claude_output = neuromap.generate_claude(max_tokens=2000)
        
        assert isinstance(claude_output, str)
        assert len(claude_output) > 0
        assert temp_path.name in claude_output
        
        # Auto-generated banner so the origin is clear.
        assert "Auto-generated by NeuroMap" in claude_output
        # Sections that orient Claude Code.
        assert "## Tech Stack" in claude_output
        assert "## Entry Points" in claude_output
        assert "## Project Structure" in claude_output
        
        # Check token budget.
        encoding = tiktoken.get_encoding("cl100k_base")
        token_count = len(encoding.encode(claude_output))
        assert token_count <= 2000
